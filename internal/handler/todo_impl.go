...
func (h *todoHandler) CreateTodo() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if err := h.todoUsecase.CreateTodo(req.Title); err != nil {
			return c.JSON(http.StatusBadRequest, echo.Map{"error": err.Error()})
		}

		return c.JSON(http.StatusCreated, echo.Map{"message": "Todoが作成されました"})
	}
}
...
func (h *todoHandler) UpdateTodo() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if err := h.todoUsecase.UpdateTodo(uint(id), req.Title, req.Completed); err != nil {
			return c.JSON(http.StatusBadRequest, echo.Map{"error": err.Error()})
		}

		return c.JSON(http.StatusOK, echo.Map{"message": "Todoが更新されました"})
	}
}
...
func (h *todoHandler) DeleteTodo() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		return c.JSON(http.StatusOK, echo.Map{"message": "Todoが削除されました"})
	}
}
